...
func normalizeAnyTLSPath(path string) string {
	path = strings.ToLower(strings.TrimSpace(path))
	path = strings.TrimPrefix(path, "/")
	return path
}

func isAnyTLSAlias(transportProtocol, path string) bool {
...
func (c *APIClient) ReportNodeOnlineUsers(onlineUserList *[]api.OnlineUser) error {
	...
	for i, user := range *onlineUserList {
		...
		if val, ok := reportOnline[user.UID]; ok {
			reportOnline[user.UID] = val + 1
		} else {
...
func (c *APIClient) ParseV2rayNodeResponse(nodeInfoResponse *NodeInfoResponse) (*api.NodeInfo, error) {
	var enableTLS bool
	var path, host, transportProtocol, serviceName, headerType, relayServer string
	var header json.RawMessage
	var relayPort, speedLimit uint64
	...
	if len(serverConf) < 6 {
		return nil, fmt.Errorf("invalid v2ray server info %q: expected at least 6 fields", nodeInfoResponse.RawServerString)
	}
	...
	for _, item := range extraServerConf {
		...
		key := strings.ToLower(strings.TrimSpace(conf[0]))
		if key == "" {
			continue
		}
		if len(conf) < 2 {
		...
		value := strings.TrimSpace(conf[1])
		switch key {
		case "path":
		...
		case "relay_server":
			relayServer = value
		case "relay_port":
			relayPort, err = strconv.ParseUint(value, 10, 32)
			if err != nil {
	...
	nodeType := c.NodeType
	effectiveTransport := transportProtocol
	...
	nodeInfo := &api.NodeInfo{
		NodeType:          nodeType,
		PanelNodeType:     c.NodeType,
		NodeID:            c.NodeID,
		Port:              port,
		SpeedLimit:        speedLimit,
		AlterID:           alterID,
		TransportProtocol: effectiveTransport,
		EnableTLS:         enableTLS,
		Path:              path,
		Host:              host,
...
func (c *APIClient) ParseUserListResponse(userInfoResponse *[]UserResponse) (*[]api.UserInfo, error) {
	...
	for _, user := range *userInfoResponse {
		if c.DeviceLimit > 0 {
			...
			if deviceLimit == 0 {
				deviceLimit = user.NodeConnector
			}
		}
